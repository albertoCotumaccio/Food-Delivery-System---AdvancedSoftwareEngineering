...
-- Module [Module]
create table `module` (
   `oid_modul`  integer  not null,
   `moduleid`  varchar(255),
   `modulename`  varchar(255),
  primary key (`oid_modul`)
);
...
alter table `module`   add index fk_module_group (`oid`), add constraint fk_module_group foreign key (`oid`) references `group` (`oid_group`);
create index `idx_module_group` on `module`(`oid`);


-- Group_Module [Group2Module_Module2Group]
alter table `module`  add column  `group_oid`  integer;
alter table `module`   add index fk_module_group_2 (`group_oid`), add constraint fk_module_group_2 foreign key (`group_oid`) references `group` (`oid_group`);
create index `idx_module_group_2` on `module`(`group_oid`);
...
create index `idx_user_2_group` on `user_2`(`group_oid`);
...
create index `idx_user_2_group_2` on `user_2`(`group_oid_2`);
...
create index `idx_rider_mean_of_transport` on `rider`(`mean_of_transport_name`);
...
create index `idx_ordine_item_ordine` on `ordine_item`(`ordine_oid`);
create index `idx_ordine_item_item` on `ordine_item`(`item_oid`);
...
create index `idx_rideruser_slot_rider` on `rideruser_slot`(`rider_oid`);
create index `idx_rideruser_slot_slot` on `rideruser_slot`(`slot_oid`);
...
create index `idx_rider_city` on `rider`(`city_name`);
...
create index `idx_restaurant_city` on `restaurant`(`city_name`);
...
create index `idx_slot_restaurant` on `slot`(`restaurant_oid`);
...
create index `idx_restaurantuser_cuisine_typ` on `restaurantuser_cuisine_type`(`restaurant_oid`);
create index `idx_restaurantuser_cuisine_t_2` on `restaurantuser_cuisine_type`(`cuisine_type_name`);
...
create index `idx_item_restaurant` on `item`(`restaurant_oid`);
...
create index `idx_ordine_customer` on `ordine`(`customer_oid`);
